...
function createAction(typeOrThunk, payloadCreator) {
    function actionCreator(...args) {
        ...
        if (typeof typeOrThunk === 'function') {
            return (dispatch, getState, dependencies) => __awaiter(this, void 0, void 0, function* () {
                try {
                    return yield typeOrThunk(...args)(dispatch, getState, dependencies);
                }
                catch (error) {
...
function createFeatureSelector(slice) {
    return (state) => {
        if (Array.isArray(slice)) {
            return slice.reduce((acc, key) => acc === null || acc === void 0 ? void 0 : acc[key], state);
        }
        else {
            return state[slice];
        }
...
function createSelector(featureSelector, selectors, projectionOrOptions) {
    ...
    return (props, projectionProps) => {
        return (state) => {
            ...
            try {
                if (isSelectorArray) {
                    const results = selectors.map((selector, i) => selector(sliceState, props === null || props === void 0 ? void 0 : props[i]));
...
function createSelectorAsync(featureSelector, selectors, projectionOrOptions) {
    ...
    return (props, projectionProps) => {
        return (state) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (isSelectorArray) {
                    const results = yield Promise.all(selectors.map((selector, i) => selector(sliceState, props === null || props === void 0 ? void 0 : props[i])));
                    if (results.some(r => r === undefined))
                        return undefined;
                    return projection(results, projectionProps);
                }
                else {
                    const result = yield selectors(sliceState, props);
                    return result === undefined
                        ? undefined
                        : projection
                            ? projection(result, projectionProps)
...
const isInstruction = (obj) => {
    return (obj === null || obj === void 0 ? void 0 : obj.type) !== undefined && (obj === null || obj === void 0 ? void 0 : obj.instance) !== undefined;
};
/**
...
function createActionHandler(config) {
     ...
     */
    const handleAction = (action, next, lock) => __awaiter(this, void 0, void 0, function* () {
        yield lock.acquire();
        ...
        try {
            if (typeof action === 'function') {
                ...
                yield action((syncAction) => __awaiter(this, void 0, void 0, function* () {
                    yield handleAction(syncAction, next, innerLock);
...
const createStarter = () => {
     ...
     */
    const exclusive = (config) => (next) => (action) => __awaiter(void 0, void 0, void 0, function* () {
        const handler = createActionHandler(config);
        const lockInstance = config.lock;
        yield handler(action, next, lockInstance);
    });
    /**
     ...
     * @returns Function - The actual middleware function that handles actions.
     */
    const concurrent = (config) => (next) => (action) => __awaiter(void 0, void 0, void 0, function* () {
        let asyncActions = [];
        const handler = createActionHandler(config);
        const lockInstance = config.lock;
    ...
    const selectStrategy = ({ dispatch, getState, dependencies, strategy, lock, stack }) => (next) => (action) => __awaiter(void 0, void 0, void 0, function* () {
...
function applyChange(initialState, { path, value }, objTree) {
    var _a;
    let currentState = Object.keys(objTree).length > 0 ? initialState : Object.assign({}, initialState);
    ...
    for (let i = 0; i < path.length; i++) {
        ...
        if (i === path.length - 1) {
            ...
            // Continue traversal
            currentObj = currentObj[key] = objTree[key] ? currentObj[key] : Object.assign({}, currentObj[key]);
            objTree = (objTree[key] = (_a = objTree[key]) !== null && _a !== void 0 ? _a : {});
        }
    }
    return currentState;
}
...
const combineReducers = (reducers) => {
     ...
     */
    return (state, action) => __awaiter(void 0, void 0, void 0, function* () {
        if (state === undefined) {
            state = yield gatherInitialState();
        }
        ...
        for (const { reducer, path } of reducerMap.values()) {
            ...
            try {
                const updatedState = yield reducer(currentState, action);
                if (currentState !== updatedState) {
                    hasChanged = true;
                    // Apply the change to the state using applyChange
                    state = yield applyChange(state, { path, value: updatedState }, modified);
                }
            }
            catch (error) {
...
const createTracker = () => {
     ...
     */
    const getStatus = (entry) => { var _a; return ((_a = entries.get(entry)) === null || _a === void 0 ? void 0 : _a.value) === true; };
    /**
     * Sets the execution status of a tracked Observable.
     */
    const setStatus = (entry, value) => { var _a; return (_a = entries.get(entry)) === null || _a === void 0 ? void 0 : _a.next(value); };
    /**
     * Marks a tracked Observable as completed.
     */
    const setCompletion = (entry) => { var _a; return (_a = entries.get(entry)) === null || _a === void 0 ? void 0 : _a.complete(); };
    /**
...
function createStore(mainModule, storeSettingsOrEnhancer, enhancer) {
    var _a;
    let main = Object.assign(Object.assign({}, defaultMainModule), mainModule);
    let modules = [];
    let sysActions = Object.assign({}, systemActions);
    ...
    if (typeof storeSettingsOrEnhancer === "function") {
        ...
        settings = Object.assign(Object.assign({}, storeSettingsOrEnhancer), defaultStoreSettings);
    }
     ...
     */
    let dispatch = (action) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            yield updateState('@global', (state) => __awaiter(this, void 0, void 0, function* () { return yield pipeline.reducer(state, action); }), action);
        }
        catch (_b) {
   ...
   */
    ...
    const unloadModule = (module, clearState = false) => {
        ...
        const promise = lock.acquire()
            ...
            .then(() => updateState("@global", (state) => __awaiter(this, void 0, void 0, function* () {
            if (clearState) {
                state = Object.assign({}, state);
                delete state[module.slice];
            }
            return yield setupReducer(state);
        })))
            .finally(() => lock.release());
     ...
     */
    const setState = (slice, value, action = systemActions.updateState()) => __awaiter(this, void 0, void 0, function* () {
        let newState;
        if (slice === undefined || typeof slice === "string" && slice == "@global") {
            // Update the whole state with a shallow copy of the value
            newState = (Object.assign({}, value));
        }
        else if (typeof slice === "string") {
            // Update the state property with the given key with a shallow copy of the value
            newState = Object.assign(Object.assign({}, currentState.value), { [slice]: Object.assign({}, value) });
        }
        else if (Array.isArray(slice)) {
     ...
     */
    const updateState = (slice, callback, action = systemActions.updateState()) => __awaiter(this, void 0, void 0, function* () {
        if (callback === undefined) {
            console.warn('Callback function is missing. State will not be updated.');
            return;
        }
        let state = getState(slice);
        let result = yield callback(state);
        yield setState(slice, result, action);
        return action;
    });
    /**
    ...
    const readSafe = (slice, callback) => {
        const promise = (() => __awaiter(this, void 0, void 0, function* () {
            try {
                yield lock.acquire(); //Potentially we can check here for an idle of the pipeline
                const state = yield getState(slice); // Get state after acquiring lock
                callback(state);
            }
            finally {
    ...
    function select(selector, defaultValue, tracker) {
        ...
        tracker === null || tracker === void 0 ? void 0 : tracker.track(selected$);
        const subscription = selected$ // Create an inner subscription
            ...
            next: (value) => {
                ...
                if (filteredValue !== undefined) {
                    ...
                    tracker === null || tracker === void 0 ? void 0 : tracker.setStatus(selected$, true);
                }
            },
            error: (err) => {
                subject.error(err);
            ...
            complete: () => {
                tracker === null || tracker === void 0 ? void 0 : tracker.complete(selected$);
                subject.complete();
            }
        });
     ...
     */
    const setupReducer = (state = {}) => __awaiter(this, void 0, void 0, function* () {
        let featureReducers = [{ slice: mainModule.slice, reducer: mainModule.reducer }, ...modules].reduce((reducers, module) => {
            let moduleReducer = module.reducer instanceof Function ? module.reducer : Object.assign({}, module.reducer);
            reducers = Object.assign(Object.assign({}, reducers), { [module.slice]: moduleReducer });
            return reducers;
        }, {});
        let reducer = combineReducers(featureReducers);
        // Define async compose function to apply meta reducers
        const asyncCompose = (...fns) => (reducer) => __awaiter(this, void 0, void 0, function* () {
            for (let i = fns.length - 1; i >= 0; i--) {
                try {
                    reducer = yield fns[i](reducer);
                }
                catch (error) {
        ...
        if (settings.enableMetaReducers && mainModule.metaReducers && mainModule.metaReducers.length) {
            try {
                reducer = yield asyncCompose(...mainModule.metaReducers)(reducer);
            }
            catch (error) {
        ...
        // Update store state
        return yield reducer(state, systemActions.updateState());
    });
    /**
    ...
    if (typeof enhancer === "function") {
        ...
        const hasMiddlewareEnhancer = enhancer.name === 'applyMiddleware' || ((_a = enhancer.names) === null || _a === void 0 ? void 0 : _a.includes('applyMiddleware'));
